...
const Flyout = () => {
  ...
  const handleDownload = () => {
    ...
    const csvContent = [
      headers,
      ...selectedItems.map((item: { name: any; url: any; }) => `${item.name},${item.url}`)
    ].join("\n");
  ...
  return (
    <div className="fixed bottom-0 w-full p-4 bg-gray-700 border-t border-white text-white flex justify-between">
      <p>{selectedItems.length} items selected</p>
      <div>
        <button onClick={() => dispatch(clearSelections())} className="mr-4 bg-red-500 px-4 py-2">
          Unselect All
        </button>
